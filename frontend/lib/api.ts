 ...
 *
 * The backend has no real auth wired in yet (see app/auth.py) - it
 * resolves the "current user" from an `X-User-Email` header, creating
 * that user on first sight. So "signing in" on the frontend just means
 * remembering an email locally and sending it on every request; there is
 * no password check. Swap this out once real auth exists on the backend.
 */

export const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL?.replace(/\/+$/, '') || 'http://localhost:8000'

const USER_EMAIL_KEY = 'spectriq_user_email'
const DEFAULT_EMAIL = 'demo@spectriq.local'
...
export function setUserEmail(email: string) {
  ...
  window.localStorage.setItem(USER_EMAIL_KEY, email.trim() || DEFAULT_EMAIL)
}
...
function authHeaders(): Record<string, string> {
  const email = getUserEmail() || DEFAULT_EMAIL
