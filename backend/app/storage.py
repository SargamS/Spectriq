...
Two backends, chosen via STORAGE_BACKEND:
  ...
  - "local" (default): saves to STORAGE_DIR on disk. Only safe when the
    API process and the Celery worker process share a filesystem - true
    in docker-compose (shared volume) or a single combined service, but
    NOT true when they're deployed as separate services/machines (e.g.
    Render's Web Service + Background Worker each get their own disk).

  - "s3": saves to an S3-compatible bucket (AWS S3, Cloudflare R2,
    Backblaze B2, DigitalOcean Spaces, MinIO...). Use this whenever the
    API and worker don't share a disk. The API uploads the file and
    stores a reference to it; the worker downloads its own local copy
    of that file before running ffmpeg/Whisper on it (those tools need
    a real local path, not a remote key).

Callers only depend on `save_upload` (called by the API on upload) and
`fetch_local_copy` (called by the worker before processing) - neither
