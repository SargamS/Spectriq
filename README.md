...
| `CLERK_JWKS_URL` | authentication | [clerk.com](https://clerk.com) — Frontend API URL + `/.well-known/jwks.json` (also needs `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`/`CLERK_SECRET_KEY` set on the frontend) |
...
- [x] Real authentication — Clerk-issued session JWTs, verified against Clerk's JWKS on every request (see `app/auth.py`); replaces the old unverified `X-User-Email` header
- [ ] Speaker diarization (who said what) — not implemented; `TranscriptChunk.speaker_label` exists as a nullable column ready for it, but no diarization pass runs today
