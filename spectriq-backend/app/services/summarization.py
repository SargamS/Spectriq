"""
Summarization via the Groq API (Chat Completions).

Groq's API is OpenAI-compatible, so this uses the official `groq` client
(same shape as the `openai` client) pointed at a Groq-hosted model. Sends
the transcript with a structured prompt requesting strict JSON output,
then parses/validates that JSON. If parsing/validation fails, retries with
an explicit correction prompt before giving up.
"""
import json
...
def _validate_shape(data: dict) -> None:
    ...
    if missing:
        raise ValueError(f"Missing required fields in Groq response: {missing}")
...
def summarize(transcript_text: str) -> dict:
    """
    Calls Groq with the transcript and returns a validated dict:
    ...
    raise SummarizationError(
        f"Groq did not return valid JSON after {1 + MAX_JSON_RETRIES} attempts: {last_error}"
    )
