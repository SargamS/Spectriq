...
(the Celery worker is a separate process - see app/workers/celery_worker.py)
"""
...
def health_check():
    ...
    does NOT touch the DB/Redis so it stays fast and always-green even
    if a downstream dependency hiccups.
    """
    return {"status": "ok"}
